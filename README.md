...
- You have some [IssueOps](https://issue-ops.github.io/docs/) commands available, that will run against your PR's base ref. Here are the available commands:
  ...
  - `.up` will run a stack update if PR is mergeable, against `dev` environment by default. Options are available
    - `<env>` will specify the environment to update
    - `--force` will force a stack update, even if PR is not mergeable. Only available to repository admins
    - `--no-rollback` will prevent the rollback of the stack in case of failure (please note that rollback only occurs on main branch by default)
